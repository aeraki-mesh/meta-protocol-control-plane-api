...
// ValidateAll checks the field values on LocalRateLimitCondition with the
// rules defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// LocalRateLimitConditionMultiError, or nil if none found.
func (m *LocalRateLimitCondition) ValidateAll() error {
	return m.validate(true)
}

func (m *LocalRateLimitCondition) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if m.GetMatch() == nil {
		err := LocalRateLimitConditionValidationError{
			field:  "Match",
	...
	if all {
		...
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, LocalRateLimitConditionValidationError{
					field:  "Match",
					reason: "embedded message failed validation",
					cause:  err,
				})
	...
	if len(errors) > 0 {
		return LocalRateLimitConditionMultiError(errors)
	}

	return nil
}

// LocalRateLimitConditionMultiError is an error wrapping multiple validation
// errors returned by LocalRateLimitCondition.ValidateAll() if the designated
// constraints aren't met.
type LocalRateLimitConditionMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m LocalRateLimitConditionMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m LocalRateLimitConditionMultiError) AllErrors() []error { return m }
